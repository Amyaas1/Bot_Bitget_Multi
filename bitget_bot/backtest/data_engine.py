...
class DataEngine():
    ...
    backtest_path=sys.path[0]
    database_base=backtest_path+'/backtest/database/'

    def __init__(self, session=ccxt.binance(), path_to_data=database_base):
    ...
    def download_data(self, symbols, timeframes, start_date='2017-01-01T00:00:00'):
        ...
        for symbol in symbols:
            print(f'-> downloding symbol {symbol} for timeframe {timeframes}')
            df = self.get_historical_from_api(symbol=symbol, timeframe=timeframes, start_date=start_date)
            if df is not None and len(df) > 0:
                
                try:
                    fileName = self.path_to_data+self.exchange_name+'/'+timeframes+'/'+symbol.replace('/','')+'.p'
                    ...
                    dbfile.close()
                    print(symbol, len(df), "Candles", timeframes, "load since the :", df.index[0],'to :',df.index[-1], 'in', fileName)
    ...
    def update_data(self, symbols, timeframes):
        ...
        for symbol in symbols:
            try:
                fileName = str(self.path_to_data)+str(self.exchange_name)+'/'+str(timeframes)+'/'+str(symbol.replace('/',''))+'.p'
                # -- Load Data --
                with open(fileName, 'rb') as file:
                    dfOrigin = pickle.load(file)

                start_date = dfOrigin.index[-1]
                dfNew = self.get_historical_from_api(symbol, timeframes, start_date)
                ...
                print(symbol, len(dfNew), "New candles", timeframes, "load since the :", dfFinal.index[0], 'in', fileName)
            except:
                print("Error on", symbol, timeframes)
